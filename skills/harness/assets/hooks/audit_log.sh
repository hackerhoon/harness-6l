...
jq -cn --arg ts "$(TS)" --arg ev "$EV" --arg t "$(tool_name)" --arg r "$(tool_ref)" \
  ...
  '{ts:$ts,event:$ev,tool:$t,ref:$r,ok:($ev=="PostToolUse"),denied:($ev=="PermissionDenied"),error:(if $err=="" then null else $err end),interrupted:$intr,duration_ms:(if $dur==0 then null else $dur end)}' \
  >> "$LEDGER" 2>/dev/null
exit 0
